...
# Dev 3 -> Dev 4 integration fix: this backend had no CORS middleware at
# all. A real content-script fetch() from the extension (initiated from the
# page's own origin, e.g. http://127.0.0.1:9797 for the QuickShop fixture)
# to this backend (http://127.0.0.1:8000) failed with a generic "Failed to
# fetch" — confirmed live, reproduced, and fixed by adding this. The other
# (non-authoritative) backend/ already had this; server/ never did.
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
